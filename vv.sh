...
echo
echo "=== Config-layer stack for arch/arm64/configs/vendor/lge/ ... ==="
echo

# Parse the device -> (device.config, board_common.config) mapping straight
# out of LGE's own build1.sh, so this can't drift from what LG's build
# actually does. Falls back to a hardcoded table if build1.sh isn't found
# or its format changed.
CONFIG_MAP_OUT="$DIFF_OUT/CONFIG_STACK.txt"
{
  echo "LG msm8996 defconfig stack (per build1.sh)"
  echo
  echo "Every device build merges FOUR files, in this order (kbuild"
  echo "multi-defconfig merge — later files override earlier ones):"
  echo "  1. vendor/lge/lge_msm8996_defconfig   (base platform, ~900 lines)"
  echo "  2. vendor/lge/<board>_common.config   (family: camera/display/PM/audio/touch)"
  echo "  3. vendor/lge/<model>.config          (machine-select flag + wifi chip variant)"
  echo "  4. vendor/lge/swan2000.config         (optional perf/undervolt tuning — the"
  echo "                                          actual point of the swan25 fork)"
  echo
  echo "Per-device mapping:"
} > "$CONFIG_MAP_OUT"

BUILD1="$LGE/build1.sh"
if [[ -f "$BUILD1" ]]; then
  # Pull out DEVICE="X" / DEVICE_DEFCONFIG=... / BOARD_DEFCONFIG=... lines in
  # file order and pair them up — simpler and more portable than parsing the
  # if/elif structure with awk.
  grep -E '\$DEVICE" = "|DEVICE_DEFCONFIG=vendor/lge/|BOARD_DEFCONFIG=vendor/lge/' "$BUILD1" \
    | sed -E 's/^\s+//' \
    | python3 -c '
import sys, re
model = None
for line in sys.stdin:
    ...
    if m:
        model = m.group(1)
        continue
    if line.startswith("DEVICE_DEFCONFIG="):
        dev = line.split("=", 1)[1]
        print(f"  {model}: device={dev}", end="")
    if line.startswith("BOARD_DEFCONFIG="):
        board = line.split("=", 1)[1]
        print(f"  board={board}")
' >> "$CONFIG_MAP_OUT"
else
  echo "  (build1.sh not found in LGE tree — inspect it manually to confirm mapping)" >> "$CONFIG_MAP_OUT"
fi

{
  echo
  echo "swan2000.config on its own is the portable, hardware-agnostic part:"
  echo "it's undervolt/overclock/TCP/debug-stripping tuning, not device wiring."
  echo "It's the piece worth cherry-picking even if you skip the rest of this port."
  echo
  echo "The three *_common.config files (alice/lucye/elsa) are NOT interchangeable"
  echo "with each other — they set different camera/display/audio/touch driver"
  echo "options per phone family, and reference Kconfig symbols (LGE_DISPLAY_*,"
  echo "SND_SOC_*, LGE_PM_*) that only exist because of the drivers copied in"
  echo "step 1 above. Pick the ONE matching your target board, not all three."
} >> "$CONFIG_MAP_OUT"

echo "Wrote device config-stack mapping to: $CONFIG_MAP_OUT"
cat "$CONFIG_MAP_OUT"
echo
...
echo "  3. Pick your target device's row from $CONFIG_MAP_OUT and merge"
echo "     lge_msm8996_defconfig + <board>_common.config + <model>.config"
echo "     (+ swan2000.config if you want the perf tuning) onto your Xiaomi defconfig"
